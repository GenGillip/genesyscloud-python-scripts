...
def getUser(token, region, groupList):
    ...
    try:
        ...
        while loop:
            ...
            userEntities = json.loads(api_response)['entities']
            pageCount = json.loads(api_response)['page_count']
            for user in userEntities:
                ...
                users.append({"id": user['id'], "name": user['name'], "email": user['username'], "groups": usrGrpString})
                userCount += 1
...
def main():
    ...
    userList, userCount  = getUser(gcxToken, region, groupList)
